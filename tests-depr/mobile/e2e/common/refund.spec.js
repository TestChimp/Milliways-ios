...
import { fetchOrdersForUser, seedOrder } from '../../../shared/refund-helpers.js';
...
test.describe('US-107 request refund', () => {
  test('refund button enabled for recent orders', async ({ screen, markScreenState }) => {
    ...
    await markScreenState('Account', 'PastOrders');
  ...
  test('refund request shows confirmation message', async ({ screen, markScreenState }) => {
    ...
    await markScreenState('Account', 'RefundConfirmation');
...
test.describe('US-107 refund eligibility — old orders', () => {
  test('refund button disabled for old orders', async ({ screen, markScreenState, seededUser }, testInfo) => {
    // @Scenario: #TS-118 refund button disabled for old orders
    const order = await seedOrder(seededUser, { ageDays: 3 });
    await signInForDemo(screen, expect, seededUser, testInfo);

    const orders = await fetchOrdersForUser(seededUser);
    expect(orders.find((row) => row.id === order.id)?.refundEligible).toBe(false);

    await openAccount(screen, expect);
    await markScreenState('Account', 'PastOrders');
    await expect(screen.getByText(`Order #${order.id}`)).toBeVisible({ timeout: 20_000 });

    const refundButton = screen.getByLabel('Request Refund').first();
    await expect(refundButton).toBeVisible({ timeout: 20_000 });
    await refundButton.tap();
    const submitted = await isVisible(
      screen,
      expect,
      screen.getByText('Your refund request was received.'),
      3_000,
    );
