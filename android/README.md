...
```properties
TESTCHIMP_PROJECT_ID=3d36a0e2-dfb8-447a-9674-3c1a4650b925
TESTCHIMP_API_KEY=your_project_api_key
# Optional — only if you must override default RUM ingest:
# TESTCHIMP_BACKEND_URL=https://…
```

Keep **`TESTCHIMP_PROJECT_ID`** / **`TESTCHIMP_API_KEY`** in sync with repo-root **`.cursor/mcp.json`**. If either is empty, RUM is skipped (Logcat tag **`Milliways`**).
...
- **Ingest URL:** When `TESTCHIMP_BACKEND_URL` is unset/empty in Gradle and process env, the RUM SDK uses its **default** ingress; set **`TESTCHIMP_BACKEND_URL`** only for a non-default host.

For SmartTests: repo-root **`tests/`** (`npm ci && npm run test:smoke`); TestChimp markdown plans under **`plans/`**. Use repo-root **`.cursor/mcp.json`** for the API key when running `testchimp` / Mobilewright reporters (do not commit keys).
...
- **Very few RUM events** often means: **RUM skipped** (missing or stale `TESTCHIMP_PROJECT_ID` / `TESTCHIMP_API_KEY` in `gradle.properties` — must match `.cursor/mcp.json`), **tests never hit** code paths that call `MilliwaysRum.emit` (smoke may not load menu / submit order), **SDK/network limits**, or **you checked the DB before the batch flushed** (events are buffered; SDK default timer is **10s** plus **flush on app background**). The Android SDK **does not log HTTP failures** for `/rum/events`—if project ID or API key is wrong, emits disappear silently; filter queries on **`environment`** (Debug defaults to **`staging`**). Mobilewright **`@testchimp/playwright` ≥ 0.1.38** issues **`v1/flush`** at end of each test so short runs still upload. If **`device.openUrl`** hangs, set **`TESTCHIMP_RUM_AUTOMATION_OPEN_URL_TIMEOUT_MS`** (default 25s) so hooks time out instead of burning the full test timeout.
